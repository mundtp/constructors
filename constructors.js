...
var Safe = function (data,instance){
	var secret = data
	var stored = instance.key
   
	this.unlock = function(input){
		if((stored === input.key) && (input === instance)){
			return secret
		}
...
var Validator = function(){
	this.email = function(string){
		var splitAt = string.split('@')
		if(splitAt.length === 1) {
			return false
		}

		var ending = splitAt[1].slice(-4)

		if ((splitAt[0].length > 1) && (ending === '.com')){
			return true
		}
		else{return false}
